<!doctype html>
<html lang="en">

<head>
    <title> Doors Lock | Bigmanply </title>
    <?php include 'include/headerlink.php' ?>
</head>

<body>
    <?php include 'include/header.php' ?>
    <div class="banner shadow">
        <img src="img/banner.png" class="img-fluid w-100" alt="Banner Image">
        <h2>Doors Lock </h2>
    </div>

    <main>


        <section class=" breadcrumb bg-light py-2">
            <div class="container">
                <nav aria-label="breadcrumb ">
                    <ol class="breadcrumb p-0 m-0">
                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                        <li class="breadcrumb-item active"><a href="#">Doors Lock</a></li>
                    </ol>
                </nav>
            </div>
        </section>

        <section class="category py-5">
            <div class="container text-center">
                <h2>Explore Our Doors Lock Categories</h2>
                <p>Find the best home products tailored for you.</p>
                <div class="row text-center">
                    <div class="col-md-3 my-4">
                        <div class="card  shadow p-3">
                            <img src="img/categories/manual.png" class="img-fluid rounded" alt="Doors Lock" style="max-width: 100%;">
                            <h5 class="mt-3"> Manual Doors Lock</h5>
                        </div>
                    </div>
                    <div class="col-md-3 my-4">
                        <div class="card shadow p-3">
                            <img src="img/categories/biomatric.png" class="img-fluid rounded" alt="Plywoods" style="max-width: 100%;">
                            <h5 class="mt-3"> Biomatric Doors Lock</h5>
                        </div>
                    </div>
                    <div class="col-md-3 my-4">
                        <div class="card shadow p-3">
                            <img src="img/categories/metald.png" class="img-fluid rounded" alt="WPC Products" style="max-width: 100%;">
                            <h5 class="mt-3">Metal Doors Lock</h5>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section class="key  py-4" style="background-color: #f3f3f3f0;">
            <div class="container text-center">
                <div class="swiper about-slider p-3">
                    <div class="swiper-wrapper">

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/finger-print-outline.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Biometric Access</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/lock-hashtag-svgrepo-com.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Keyless Entry</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/shield.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Enhanced Security</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/lock-password-unlocked.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Smart Lock Integration</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/strong-man-svgrepo-com.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Durable Metal Build</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/water-resistant.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Weather Resistant</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="swiper-slide">
                            <div class="p-3">
                                <center>
                                    <img class="img-fluid hover-zoom w-50" src="img/icons/water.png" alt="Bigmanply">
                                </center>
                                <div class="row g-2 mt-1">
                                    <div class="col-md-12">
                                        <h5 class="mb-1 fw-bold">Anti-Tamper Protection</h5>
                                    </div>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
        </section>

        <section class="products  py-4">
            <div class="container text-center">
                <h2>Doors Lock Products</h2>
                <div class="row text-center py-3">
                    <div class="col-md-3  py-2 ">
                        <div class="card shadow hover-zoom">
                            <a href="img/door-lock/lock.jpg" data-lightbox="doors-gallery" data-title="Door 1">
                                <img src="img/door-lock/lock.jpg" class=" rounded w-100" alt="door">
                            </a>
                        </div>
                    </div>
                    <div class="col-md-3  py-2 ">
                        <div class="card shadow hover-zoom ">
                            <a href="img/door-lock/lock1.jpg" data-lightbox="doors-gallery" data-title="Door 1">
                                <img src="img/door-lock/lock1.jpg" class=" rounded w-100" alt="door">
                            </a>
                        </div>
                    </div>
                    <div class="col-md-3  py-2 ">
                        <div class="card shadow hover-zoom ">
                            <a href="img/door-lock/lock2.jpg" data-lightbox="doors-gallery" data-title="Door 1">
                                <img src="img/door-lock/lock2.jpg" class=" rounded w-100" alt="door">
                            </a>
                        </div>
                    </div>
                </div>

            </div>
        </section>

        <section class=" bg-light py-4">
            <div class="container text-center">
                <h2>Metal Doors Lock Catalogue</h2>
                <div class="row text-center py-3">
                    <div class="col-md-12 col-12 py-2 ">
                        <iframe src="pdf/hardware.pdf" width="100%" height="500px" frameborder="0"></iframe>
                    </div>
                </div>
            </div>
        </section>

        <section class="contact-section py-5 ">
            <div class="container text-center">
                <h2 class="mb-3">Get in Touch</h2>
                <p class="mb-4">For any inquiries about our services, feel free to contact us directly.</p>

                <div class="row justify-content-center">
                    <div class="col-md-4">
                        <div class="card shadow p-3">
                            <h5 class="mb-2">📞 Call Us</h5>
                            <p><a href="tel:+919162425039" class="text-dark text-decoration-none">+91 9162425039</a>
                                <a href="tel:+917004515210" class="text-dark text-decoration-none">+91 7004515210</a>
                            </p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card shadow p-3">
                            <h5 class="mb-2">📧 Email Us</h5>
                            <p><a href="mailto:info@bigmanply.com" class="text-dark text-decoration-none">info@bigmanply.com</a></p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card shadow p-3">
                            <h5 class="mb-2">📍 Visit Us</h5>
                            <p>123 Business Road, Jharkhand, India</p>
                        </div>
                    </div>
                </div>

                <div class="mt-4">
                    <a href="https://wa.me/917004515210" class="btn btn-success gg"><i class="fa-brands fa-whatsapp"></i> Chat on WhatsApp</a>
                </div>
            </div>
        </section>


    </main>
    <?php include 'include/footer.php' ?>
</body>

</html>